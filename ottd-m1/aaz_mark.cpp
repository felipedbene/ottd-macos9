/* aaz_mark.cpp — R1-180 Classic ctor-chain bisection marker (auto-generated
...
__attribute__((constructor)) static void aaz_mark_ctor(void)
{
    FILE *f = fopen("premain.txt", "a");
    if (f != nullptr) { fprintf(f, "aaz mb=%ld fm=%ld\n", b2_maxblock(), b2_freemem()); fclose(f); }
}
