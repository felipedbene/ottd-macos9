// BISECT-B no-fio: identical link/static-init graph as ottd-b1, but main() only logs+returns.
// Purpose: decide ctor-vs-runtime. If ottd-bB.txt appears on the share, ALL global
...
int main()
{
    ottd_log_init("ottd-bB.txt");
    ottd_log("=== BISECT-B no-fio: main() reached -> ALL global ctors ran OK ===");
    ottd_log("free heap = %ld bytes", (long)FreeMem());
    ottd_log("keep_b1 = %p (graph retained, render NOT called)", keep_b1);
    ottd_log("=== BISECT-B no-fio done ===");
    ottd_log_close();
    return 0;
}
