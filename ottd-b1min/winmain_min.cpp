// B1-min: identical link/static-init graph as ottd-b1, but main() only logs+returns.
// Purpose: decide ctor-vs-runtime. If ottd-b1min.txt appears on the share, ALL global
...
int main()
{
    ottd_log_init("ottd-b1min.txt");
    ottd_log("=== B1-min: main() reached -> ALL global ctors ran OK ===");
    ottd_log("free heap = %ld bytes", (long)FreeMem());
    ottd_log("keep_b1 = %p (graph retained, render NOT called)", keep_b1);
    ottd_log("=== B1-min done ===");
    ottd_log_close();
    return 0;
}
