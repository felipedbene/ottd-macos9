...
int main()
{
    MaxApplZone();   // grow the application heap zone to fill the SIZE partition (16 MB).
                     // Without this, Retro68 leaves the heap tiny and big allocations spill
                     // into temporary memory (movable handles) -> intermittent Type-2 crashes.
    ottd_log_init("ottd-b1.txt");                 // FIRST thing: catch any reach-of-main
    ottd_log("=== B1 real-pipeline demo: main() reached ===");
    InitGraf(&qd.thePort); InitFonts(); InitWindows(); InitMenus(); InitCursor();
    ottd_log("Toolbox inited; FreeMem=%ld MaxBlock=%ld TempFreeMem=%ld",
             (long)FreeMem(), (long)MaxBlock(), (long)TempFreeMem());

    Rect wr; SetRect(&wr, 20, 40, 20 + FB_W, 40 + FB_H);
    WindowPtr win = NewCWindow(NULL, &wr, "\pOpenTTD B1: real viewport pipeline",
    ...
    ottd_log("framebuffer ok; calling b1_render ...");

    int rc = b1_render(fb, (int)rowBytes, FB_W, FB_H);
    ...
    ottd_log("=== exiting via ExitToShell (skip fragile static dtors) ===");
    ottd_log_close();
    ExitToShell();   // terminate without running C++ static destructors (exit-time teardown
                     // hits static-destruction-order issues, e.g. ~BlitterFactory/~vector)
    return 0;
}
