...
void HandleMouseEvents()
{
	...
	static R1SteadyClock::time_point hover_time = {};
	...
	if (_settings_client.gui.hover_delay_ms > 0) {
		if (!_cursor.in_window || click != MC_NONE || mousewheel != 0 || _left_button_down || _right_button_down ||
			...
			hover_time = R1SteadyClock::now();
			_mouse_hovering = false;
		} else if (!_mouse_hovering) {
			if (R1SteadyClock::now() > hover_time + std::chrono::milliseconds(_settings_client.gui.hover_delay_ms)) {
				click = MC_HOVER;
				_input_events_this_tick++;
...
void UpdateWindows()
{
	static R1SteadyClock::time_point last_time = R1SteadyClock::now();
	uint delta_ms = std::chrono::duration_cast<std::chrono::milliseconds>(R1SteadyClock::now() - last_time).count();

	if (delta_ms == 0) return;

	last_time = R1SteadyClock::now();
